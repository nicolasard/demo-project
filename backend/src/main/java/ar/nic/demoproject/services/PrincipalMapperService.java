...
public class PrincipalMapperService {
    ...
    public PrincipalMapperService(final UserProfileRepository userProfileRepository,
                                  final CustomJwtTokenUtils jwtTokenUtils, JwtValidator jwtValidator) {
     ...
     */
    public Mono<String> authenticate(final AuthenticationType authenticationType, final String jwtToken){
    ...
    private Mono<UserProfile> getUserFromDB(UserProfile userProfile){
        return userProfileRepository.findByEmail(Mono.just(userProfile.getEmail()))
                .switchIfEmpty(userProfileRepository.save(new UserProfile(userProfile.getFullName(), userProfile.getEmail())));
    }
}
